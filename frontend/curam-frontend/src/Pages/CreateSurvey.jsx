...
export default function CreateSurvey() {
  ...
  useEffect(() => {
    if (survey) {
      ...
      setStatus(survey.status || "Draft"); // pick from backend or default Draft
  ...
  // 🔥 Single method that accepts status parameter
  const handleSubmitSurvey = async (submitStatus) => {
    ...
    const surveyData = {
      ...
      status: submitStatus || status, // 👈 Draft or Final
    ...
    <div style={{ padding: "20px" }}>
      ...
      {showForm && (
        <div>
          ...
          <button
            onClick={() => handleSubmitSurvey("Final")}
            style={{
